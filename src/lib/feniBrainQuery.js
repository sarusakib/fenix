const PHRASE_MAP = new Map([
  ...
  ['fire service', 'ফায়ার সার্ভিস'], ['ambulance chai', 'অ্যাম্বুল্যান্স চাই'],
...
const TOKEN_MAP = new Map([
  ...
  ['emergency', 'জরুরি'], ['joruri', 'জরুরি'], ['ambulance', 'অ্যাম্বুল্যান্স'], ['police', 'পুলিশ'], ['fire', 'আগুন'],
...
const INTENT_DEFS = [
  ['START_BUSINESS',['ব্যবসা শুরু','ব্যবসা করতে চাই','দোকান দিতে চাই','start a business','business start','কিভাবে শুরু','কিভাবে ব্যবসা']],
  ['INVEST',['বিনিয়োগ','investment','invest','বিনিয়োগকারী','funding','capital','অর্থায়ন']],
  ['FIND_SUPPLIER',['সরবরাহকারী','supplier','suppliers','পাইকার','পাইকারি','wholesale','wholesaler','distributor']],
  ['FIND_BUSINESS',['দোকান','business','businesses','shop','কোন ব্যবসা','ব্যবসা কোথায়']],
  ['FIND_SERVICE',['সেবা','service','হাসপাতাল','ক্লিনিক','ডাক্তার','স্কুল','college','পুলিশ','ফায়ার']],
  ['LOCATION_INFO',['উপজেলা','ইউনিয়ন','ওয়ার্ড','পৌরসভা','গ্রাম','মৌজা','বাজার','এলাকা','কোথায়','location','area']],
  ['BUSINESS_REGISTRATION',['ট্রেড লাইসেন্স','নিবন্ধন','registration','license','licence']],
  ['DUE_DILIGENCE',['যাচাই','ভেরিফাই','verify','verification','due diligence','মালিকানা','ownership','প্রমাণ']],
  ['BUSINESS_COMPARISON',['তুলনা','compare','comparison','কোনটা ভালো','difference','পার্থক্য']],
  ['CALCULATOR',['হিসাব','calculator','ক্যালকুলেট','কত টাকা লাগবে','খরচ কত','profit','লাভ হিসাব']],
  ['MARKET_RESEARCH',['বাজার গবেষণা','market research','চাহিদা','demand','competition','প্রতিযোগিতা','বাজার কেমন']],
  ['GENERAL_GUIDANCE',['কিভাবে','how','guide','guidance','কি করব','what should i do']],
];

const BUDGET_UNITS = new Map([
  ['lakh',100000], ['লক্ষ',100000], ['লাখ',100000], ['crore',10000000], ['crores',10000000], ['কোটি',10000000],
...
export function detectLanguage(input) {
  ...
  if(latin){
    ...
    if(/\\b(ami|amar|feni|kothay|kivabe|chai|korbo|diye|te|er)\\b/.test(lower)) return 'banglish';
    return 'en';
  }
  return 'unknown';
}
...
function parseAmount(raw) {
  const clean=toArabicDigits(raw).replace(/[৳$€£]/g,'').replace(/,/g,'').trim();
...
export function extractBudgetBDT(input) {
  ...
  const patterns=[
    /(?:৳|tk|bdt)?\\s*([0-9]+(?:\\.[0-9]+)?)\\s*(কোটি|crore|crores|লক্ষ|লাখ|lakh|হাজার|thousand|k)\\b/iu,
    /(?:budget|বাজেট)\\s*(?:of|=|:)?\\s*(?:৳|tk|bdt)?\\s*([0-9]+(?:\\.[0-9]+)?)\\s*(কোটি|crore|crores|লক্ষ|লাখ|lakh|হাজার|thousand|k)?\\b/iu,
    /(?:৳|tk|bdt)\\s*([0-9]{3,})\\b/iu,
  ];
...
export function detectFeniBrainIntent(input) {
  ...
  let best={key:'GENERAL_GUIDANCE',score:0,index:INTENT_DEFS.length};
  INTENT_DEFS.forEach(([key,terms],index)=>{
    const score=terms.reduce((sum,term)=>{
      const t=term.toLowerCase();
      return sum+(text.includes(t)?1+Math.min(t.length/25,2):0);
    },0);
    if(score>best.score||(score===best.score&&index<best.index)) best={key,score,index};
  });
  return best.key;
}
...
export function extractEntities(input) {
  ...
  ].filter(([,bn])=>lower.includes(bn.toLowerCase())).map(([en])=>en);
  return {locations,budget_bdt:extractBudgetBDT(input),keywords:parsed.meaningfulTokens.slice(0,20)};
}
...
export function classifyFeniBrainQuestion(input) {
  ...
  let scope='general';
  if(start) scope='start'; else if(investment) scope='investment'; else if(commerce) scope='commerce'; else if(business) scope='business'; else if(local) scope='local';
  return {
    ...parsed,scope,local,current,highStakes,business,commerce,investment,start,
    language:detectLanguage(input),intentKey:detectFeniBrainIntent(input),budgetBDT:extractBudgetBDT(input),
    entities:extractEntities(input),general:scope==='general'&&!current&&!highStakes,
  };
}
