...
export function classifyFeniBrainQuestion(input) {
  ...
  const currentTerms = [
    'আজ', 'এখন', 'সর্বশেষ', 'বর্তমান', 'হালনাগাদ', 'আপডেট', 'নোটিশ', 'খবর',
    'latest', 'today', 'now', 'current', 'updated', 'update', 'notice', 'news',
  ];

  const highStakesTerms = [
    'আইন', 'আইনি', 'legal', 'law', 'চিকিৎসা', 'ডাক্তার', 'medicine', 'medical',
    'স্বাস্থ্য', 'health', 'জরুরি', 'emergency', 'বিনিয়োগ', 'investment', 'return',
    'লাভ', 'ক্ষতি', 'loan', 'ঋণ', 'tax', 'কর', 'নিরাপত্তা', 'security',
  ];

  const localTerms = [
    'ফেনী', 'ফেনীতে', 'সদর', 'ছাগলনাইয়া', 'দাগনভূঞা', 'ফুলগাজী', 'পরশুরাম',
    'সোনাগাজী', 'উপজেলা', 'ইউনিয়ন', 'ওয়ার্ড', 'বাজার', 'মৌজা', 'গ্রাম', 'এলাকা',
  ];

  const businessTerms = [
    'ব্যবসা', 'supplier', 'সরবরাহকারী', 'পাইকারি', 'দোকান', 'business', 'shop',
    'wholesale', 'manufacturer', 'distributor',
  ];

  const commerceTerms = [
    'পণ্য', 'product', 'cart', 'order', 'অর্ডার', 'দাম', 'price', 'shop local',
    'delivery', 'ডেলিভারি', 'return', 'রিটার্ন', 'seller', 'বিক্রি',
  ];

  const investmentTerms = [
    'বিনিয়োগ', 'investment', 'invest', 'investor', 'বিনিয়োগকারী', 'funding',
    'capital', 'অর্থায়ন', 'লাভ', 'return',
  ];

  const startTerms = [
    'ব্যবসা শুরু', 'business start', 'start a business', 'শুরু করতে চাই',
    'কিভাবে ব্যবসা', 'কিভাবে শুরু',
  ];

  const hasAny = (terms) => terms.some((term) => text.includes(term));

  const current = hasAny(currentTerms);
  const local = hasAny(localTerms);
  const business = hasAny(businessTerms);
  const commerce = hasAny(commerceTerms);
  const investment = hasAny(investmentTerms);
  const start = hasAny(startTerms);
  const highStakes = hasAny(highStakesTerms);
...
function parseAmount(value) {
  const clean = toArabicDigits(value).replace(/[৳$€£]/g, '').replace(/,/g, '').trim();
...
export function detectFeniBrainIntent(input) {
  ...
  for (const [key, terms] of definitions) {
    const score = terms.reduce((sum, term) => (
      text.includes(term.toLowerCase()) ? sum + 1 + Math.min(term.length / 25, 2) : sum
    ), 0);
