...
export default function FenixUiTranslator(){
  ...
  useEffect(()=>{
    ...
    const run=()=>{
      ...
      nodes.forEach(n=>{if(skip(n))return;const v=n.nodeValue||'';const x=tr(v);if(x!==v)n.nodeValue=x})
      document.querySelectorAll('input[placeholder],textarea[placeholder],[aria-label],[title]').forEach(el=>{
        ...
        for(const attr of ['placeholder','aria-label','title']){
          const v=el.getAttribute(attr); if(v){const x=tr(v); if(x!==v) el.setAttribute(attr,x)}
        }
      })
    }
    let raf=0
