          ...
          <div className="grid gap-5 lg:grid-cols-[.34fr_1fr]">
            <div className="rounded-3xl border border-[var(--fx-border)] bg-[var(--fx-primary-soft)] p-5">
              <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] text-[var(--fx-primary-strong)]">
                <span className="grid h-8 w-8 place-items-center rounded-xl bg-[var(--fx-bg)]">
              ...
              <div className="mt-5 flex items-center gap-2 text-xs font-semibold text-[var(--fx-muted)]">
                <CheckCircle size={16} className="text-[var(--fx-primary)]" /> {liveServices.length} available
              </div>
            </div>
                ...
                {liveServices.map((service) => {
                  ...
                  return (
                    <button
                      ...
                      className={'fenix-interactive group flex min-h-16 items-center gap-3 rounded-2xl border px-3.5 py-3 text-left ' + (active ? 'border-[var(--fx-primary)]/25 bg-[var(--fx-primary-soft)]' : 'border-[var(--fx-border)] bg-[var(--fx-bg)]/45')}
                    >
              ...
              {selectedService && guide && (
                <div className="mt-3 rounded-3xl border border-[var(--fx-primary)]/20 bg-[var(--fx-primary-soft)] p-5 sm:p-6" role="region" aria-label={selectedService.label + ' guide'}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[.16em] text-[var(--fx-primary-strong)]">Next step</p>
                    ...
                    <button type="button" onClick={() => setSelectedId(null)} aria-label="Close guide" className="grid h-9 w-9 place-items-center rounded-xl border border-[var(--fx-border)] bg-[var(--fx-bg)]"><X size={17}/></button>
                  </div>
                  <p className="mt-4 text-sm leading-6 text-[var(--fx-muted)]">{guide.introBn}</p>
                  <ol className="mt-4 grid gap-2 sm:grid-cols-2">
                    {guide.stepsBn.slice(0, 4).map((step, index) => (
                      <li key={step} className="flex gap-2 rounded-2xl border border-[var(--fx-border)] bg-[var(--fx-bg)]/65 p-3 text-xs font-semibold">
                        <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[var(--fx-primary-soft)] text-[10px] font-black text-[var(--fx-primary-strong)]">{index + 1}</span>
                        <span className="pt-0.5">{step}</span>
                      </li>
                    ))}
                  </ol>
                  <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                    <div className="flex-1 rounded-2xl border border-[var(--fx-border)] bg-[var(--fx-bg)]/65 p-3 text-xs leading-5 text-[var(--fx-muted)]">
                      <strong className="text-[var(--fx-text)]">মনে রাখবেন:</strong> {guide.noteBn}
                    </div>
                    <Link href={selectedService.href!} onClick={() => onNavigate?.()} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[var(--fx-primary-strong)] px-5 text-sm font-bold text-white">
...
function CategoryTab({ group, active, onClick }: { group: FeniXServiceGroup; active: boolean; onClick: () => void }) {
  ...
  return (
    <button type="button" role="tab" id={'fenix-service-tab-' + group.id} aria-selected={active} onClick={onClick}
      className={'flex min-h-12 items-center justify-center gap-2 rounded-xl border px-2.5 text-center transition ' + (active ? 'border-[var(--fx-primary)]/20 bg-[var(--fx-primary-soft)] text-[var(--fx-text)] shadow-sm' : 'border-[var(--fx-border)] bg-[var(--fx-bg)]/40 text-[var(--fx-muted)] hover:bg-[var(--fx-bg)]')}>
      <Icon size={17} weight={active ? 'duotone' : 'regular'} />
      <span className="min-w-0"><span className="block truncate text-[11px] font-black">{group.label}</span><span className="block truncate text-[9px] opacity-60">{group.labelBn}</span></span>
    </button>
  )
}
