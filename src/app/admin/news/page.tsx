  ...
  <div className="mt-7 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
   <section className="rounded-[2rem] border border-[var(--fx-border)] bg-[var(--fx-surface)] p-5 sm:p-7">
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2">
      <Field label="English title"><input value={form.title_en} onChange={e=>patch('title_en',e.target.value)} className="input" /></Field>
      <Field label="Bangla title"><input value={form.title_bn} onChange={e=>patch('title_bn',e.target.value)} className="input" /></Field>
      <Field label="Slug"><input value={form.slug} onChange={e=>patch('slug',e.target.value)} className="input" placeholder="story-slug" /></Field>
      <Field label="Category"><select value={form.category} onChange={e=>patch('category',e.target.value)} className="input">{cats.map(c=><option key={c[0]} value={c[0]}>{c[1]}</option>)}</select></Field>
      <Field label="Status"><select value={form.status} onChange={e=>patch('status',e.target.value)} className="input"><option value="draft">Draft</option><option value="review">Review</option><option value="published">Published</option><option value="archived">Archived</option></select></Field>
      <Field label="Verification"><select value={form.verification_status} onChange={e=>patch('verification_status',e.target.value)} className="input"><option value="official_source">Official source</option><option value="editor_reviewed">Editor reviewed</option><option value="reported">Reported</option><option value="unverified">Unverified</option></select></Field>
      <Field label="Source name"><input value={form.source_name||''} onChange={e=>patch('source_name',e.target.value)} className="input" /></Field>
      <Field label="Source URL"><input value={form.source_url||''} onChange={e=>patch('source_url',e.target.value)} className="input" placeholder="https://..." /></Field>
      <Field label="Excerpt (English)"><textarea value={form.excerpt_en||''} onChange={e=>patch('excerpt_en',e.target.value)} rows={3} className="input" /></Field>
      <Field label="Excerpt (Bangla)"><textarea value={form.excerpt_bn||''} onChange={e=>patch('excerpt_bn',e.target.value)} rows={3} className="input" /></Field>
