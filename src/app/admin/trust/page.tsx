...
export default function AdminTrustPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!active) return
      if (c.error || r.error || b.error) { setError('Trust queue could not be loaded completely.'); return }
      setClaims(c.data ?? [])
      setReviews(r.data ?? [])
      setReports(b.data ?? [])
      const ids = [...new Set([...(c.data ?? []), ...(r.data ?? []), ...(b.data ?? [])].map(x => x.business_id).filter(Boolean))]
      if (ids.length) {
  ...
  async function act(kind: 'claim' | 'review' | 'report', id: string, status: string) {
    setBusy(kind + id)
    setError('')
    const s = createClient()
  ...
  const renderQueue = (title: string, rows: Row[], kind: 'claim' | 'review' | 'report') => (
        ...
        {rows.length ? rows.map(row => (
          <article key={row.id} className="rounded-2xl border border-black/10 bg-black/[.018] p-4 dark:border-white/[.08] dark:bg-white/[.02]">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              {row.status === 'pending' && <div className="flex gap-2">{kind === 'report' ? <><ActionButton onClick={() => void act(kind, row.id, 'resolved')} disabled={busy === kind + row.id} good>Resolve<Check size={15} /></ActionButton><ActionButton onClick={() => void act(kind, row.id, 'dismissed')} disabled={busy === kind + row.id}>Dismiss<X size={15} /></ActionButton></> : <><ActionButton onClick={() => void act(kind, row.id, kind === 'claim' ? 'approved' : 'published')} disabled={busy === kind + row.id} good> {kind === 'claim' ? 'Approve' : 'Publish'} <Check size={15} /></ActionButton><ActionButton onClick={() => void act(kind, row.id, 'rejected')} disabled={busy === kind + row.id}>Reject<X size={15} /></ActionButton></>}</div>}
            </div>
