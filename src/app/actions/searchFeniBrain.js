...
export async function searchFeniBrain(query) {
  if (typeof query !== 'string') return { success: false, error: 'Invalid search query.' }
  ...
  try {
    ...
    const [{ data: terms, error: termsError }, { data: locations, error: locationError }] =
      await Promise.all([
        supabase.from('fenix_brain_query_terms').select('intent_key, term, weight').limit(2000),
        supabase.rpc('search_feni_brain_locations', { query_text: normalizedQuery, match_count: 10 }),
      ])

    if (termsError) console.error('Feni Brain intent terms retrieval failed:', { code: termsError.code })
    if (locationError) console.error('Feni Brain location retrieval failed:', { code: locationError.code })
    ...
    for (const item of terms ?? []) {
      ...
      intentScores.set(item.intent_key, current + Number(item.weight ?? 1) + phraseBonus)
    }
    ...
    const keywordResult = await supabase.rpc('keyword_feni_brain_chunks', {
      query_text: keywordQuery || normalizedQuery,
      match_count: MAX_RESULTS,
    })

    if (keywordResult.error) {
      console.error('Feni Brain keyword retrieval failed:', { code: keywordResult.error.code })
    }
    ...
    try {
      ...
      if (!semanticResult.error && Array.isArray(semanticResult.data)) semantic = semanticResult.data
    } catch (error) {
    ...
    const semanticMap = new Map(semantic.map((row) => [row.id, Number(row.similarity ?? 0)]))
    const keywordMap = new Map(keyword.map((row) => [row.id, Number(row.similarity ?? 0)]))
    const merged = new Map()

    for (const row of [...semantic, ...keyword]) {
      const semanticSimilarity = semanticMap.get(row.id) ?? 0
      const keywordSimilarity = keywordMap.get(row.id) ?? 0
      const normalizedSemantic = Math.max(0, Math.min((semanticSimilarity - 0.35) / 0.65, 1))
      const hybridScore = semanticSimilarity > 0
      ...
      merged.set(row.id, {
        ...
        retrieval_method:
          semanticSimilarity > 0 && keywordSimilarity > 0 ? 'hybrid' :
          semanticSimilarity > 0 ? 'semantic' : 'keyword',
      })
    }
    ...
    return { success: false, error: 'Feni Brain বর্তমানে unavailable.' }
  }
}
