...
export default function NewProductPage() {
  ...
  useEffect(() => {
    ...
    async function loadForm() {
      try {
        ...
        if (userError || !user) {
          router.replace('/login?next=/commerce/seller/products/new')
          return
        }
        ...
        const currentVendor = vendorResult.data as Vendor
        ...
        if (categoryResult.error) {
          ...
          setError('Product categories load করা যায়নি।')
          return
        }

        setCategories((categoryResult.data ?? []) as Category[])
      } catch (loadError) {
        console.error('New product page failed:', {
          name:
  ...
  function handleEnglishNameChange(value: string) {
    ...
    if (!slug || slug === createSlug(nameEn)) {
      setSlug(createSlug(cleanValue))
    }
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!vendor) {
      setError('Seller information পাওয়া যায়নি।')
      return
    }
    ...
    const cleanNameBn = nameBn.trim().slice(0, 200)
    const cleanNameEn = nameEn.trim().slice(0, 200)
    const cleanSlug = createSlug(slug || nameEn || nameBn)
    const cleanDescriptionBn = descriptionBn.trim().slice(0, 4000)
    const cleanDescriptionEn = descriptionEn.trim().slice(0, 4000)
    const cleanSku = sku.trim().slice(0, 100)
    ...
    if (!cleanNameBn && !cleanNameEn) {
      setError('বাংলা অথবা English product name দিন।')
      return
    }

    if (!cleanSlug) {
      setError('Product slug তৈরি করা যায়নি।')
      return
    }

    if (!Number.isFinite(numericPrice) || numericPrice < 0) {
      setError('Valid product price দিন।')
      return
    }
    ...
    try {
      ...
      const { data: createdProduct, error: insertError } =
        await supabase
          ...
          .insert({
            ...
            is_active: false,
            is_featured: false,
            allow_guest_purchase: allowGuestPurchase,
          })
          .select('id')
          .single()
      ...
      setSuccess('Product draft সফলভাবে তৈরি হয়েছে।')

      setTimeout(() => {
        router.push('/commerce/seller')
      }, 700)
    } catch (submitError) {
  ...
  return (
    <main className="min-h-screen w-full overflow-x-clip bg-[#f7faf9] text-[#0b1736] dark:bg-[#030506] dark:text-white">
      <Navbar />

      <section className="mx-auto w-full max-w-4xl px-4 pb-20 pt-8 sm:px-6 lg:px-8 lg:pt-10">
        <button
          type="button"
          onClick={() => router.push('/commerce/seller')}
          className="
        ...
        <div className="mt-10">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-[#0b1736]/55 dark:text-white/45">
            Product প্রথমে draft হিসেবে তৈরি হবে। Image, inventory এবং
            publishing workflow পরবর্তী ধাপে যুক্ত হবে।
          </p>
        </div>
        ...
        >
          ...
          <div className="mt-5">
            <Field
              ...
              label="Product slug"
              value={slug}
              onChange={(value) => setSlug(createSlug(value))}
              placeholder="feni-local-honey"
              maxLength={160}
            />

            <p className="mt-2 text-xs text-[#0b1736]/40 dark:text-white/35">
              Shareable product URL-এর জন্য ব্যবহার হবে।
            </p>
          </div>
              ...
              >
                ...
                {categories.map((category) => (
                  <option key={category.id} value={category.id}>
                    {category.name_bn || category.name_en || 'Category'}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="mt-5">
            ...
            <textarea
              ...
              value={descriptionBn}
              onChange={(event) =>
                setDescriptionBn(event.target.value.slice(0, 4000))
              }
              rows={5}
              maxLength={4000}
              ...
              className="
                ...
                border border-[#0b1736]/10
                bg-white px-4 py-3 text-sm leading-6
                text-[#0b1736] outline-none
          ...
          <div className="mt-5">
            ...
            <textarea
              ...
              value={descriptionEn}
              onChange={(event) =>
                setDescriptionEn(event.target.value.slice(0, 4000))
              }
              rows={5}
              maxLength={4000}
          ...
          <label
            className="
              ...
              rounded-xl border border-[#0b1736]/10
              bg-[#0b1736]/[0.025] p-4
          ...
          >
            <input
              type="checkbox"
              checked={allowGuestPurchase}
              onChange={(event) =>
                setAllowGuestPurchase(event.target.checked)
              }
              className="mt-1 h-4 w-4 accent-[#008080]"
            />
            ...
            <span>
              ...
              <span className="mt-1 block text-xs leading-5 text-[#0b1736]/45 dark:text-white/40">
                Customer FeniX account ছাড়াই এই product order করতে
                পারবে।
              </span>
            </span>
          </label>
          ...
          >
            ...
            <p className="text-xs leading-5 text-[#0b1736]/55 dark:text-white/45">
              এই ধাপে product শুধুমাত্র <strong>draft</strong> হিসেবে
              তৈরি হবে। সরাসরি public listing বা stock পরিবর্তনের
              permission নেই।
            </p>
          </div>
          ...
          <button
            ...
            disabled={submitting}
            className="
              mt-7 inline-flex w-full items-center
