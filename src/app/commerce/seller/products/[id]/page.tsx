...
import ProductImageManager from '@/components/commerce/ProductImageManager'
import { createClient } from '@/utils/supabase/client'

type ProductState = { id: string; vendor_id: string; name_bn: string; name_en: string; price: number; status: string; is_active: boolean }
...
export default function SellerProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error: productError } = await supabase.from('products').select('id, vendor_id, name_bn, name_en, price, status, is_active').eq('id', id).eq('vendor_id', vendor.id).maybeSingle()
      if (!active) return
      if (productError || !data) { setError('Product পাওয়া যায়নি।'); setLoading(false); return }
      const [{ data: stock }, { data: imageData }] = await Promise.all([
        supabase.from('inventory').select('quantity, low_stock_threshold').eq('product_id', id).maybeSingle(),
        supabase.from('product_images').select('id, storage_bucket, storage_path, alt_text_bn, alt_text_en, sort_order, is_primary').eq('product_id', id).order('sort_order', { ascending: true }),
      ])
