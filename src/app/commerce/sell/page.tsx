...
function normalizeVendorProfile(
vendor: VendorProfileQueryRow,
): VendorProfile {
return {
id: vendor.id,
...
async function loadSellerState() {
  try {
    ...
    if (vendorError) {
      ...
      /*
       * Supabase's generated Database contract in the
       * current project can resolve this relation as
       * `never` during production type checking.
       *
       * Normalize the selected row at this boundary
       * instead of spreading `any` through the page.
       */
      const vendor =
        vendorData as unknown as VendorProfileQueryRow

      setExistingVendor(
        normalizeVendorProfile(vendor),
      )
    }
  } catch (loadError) {
    console.error(
      'Seller page load failed:',
      {
        name:
...
const cleanDisplayNameBn =
  displayNameBn.trim().slice(0, 200)

const cleanDisplayNameEn =
  displayNameEn.trim().slice(0, 200)

const cleanDescriptionBn =
  descriptionBn.trim().slice(0, 2000)

const cleanDescriptionEn =
  descriptionEn.trim().slice(0, 2000)
...
if (cleanDisplayName.length < 2) {
  setError(
    'Seller name কমপক্ষে ২ অক্ষরের হতে হবে।',
  )
  return
}
...
try {
  ...
  const vendorPayload: VendorProfileInsert = {
    ...
    display_name_bn:
      cleanDisplayNameBn || null,
    display_name_en:
      cleanDisplayNameEn || null,
    description_bn:
      cleanDescriptionBn || null,
    description_en:
      cleanDescriptionEn || null,
    phone: cleanPhone,
  }

  const {
    data: createdVendorData,
    error: insertError,
  } = await supabase
    .from('vendor_profiles')
  ...
  const createdVendor =
    createdVendorData as unknown as VendorProfileQueryRow
