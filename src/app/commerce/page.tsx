...
import { ArrowRight, ShoppingBag, Storefront, ShieldCheck } from '@phosphor-icons/react/dist/ssr'
...
export default async function CommercePage({ searchParams }: CommercePageProps) {
  ...
  if (q) {
    ...
    query = query.or(
      [
        'name_bn.ilike.%' + escaped + '%',
        'name_en.ilike.%' + escaped + '%',
        'description_bn.ilike.%' + escaped + '%',
        'description_en.ilike.%' + escaped + '%',
        'vendor_display_name.ilike.%' + escaped + '%',
      ].join(','),
    )
  }
  ...
  const products = ((data || []) as unknown as PublicProductRow[]).map((product) => {
    const imageUrl =
      product.image_bucket && product.image_path
        ? supabase.storage.from(product.image_bucket).getPublicUrl(product.image_path).data
            .publicUrl
        : null
    ...
    return {
      ...
      vendor: {
        display_name: product.vendor_display_name,
    ...
    <main className="min-h-screen bg-[#f7faf9] text-[#0b1736] dark:bg-[#030506] dark:text-white">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-20 pt-8 sm:px-6 lg:px-8">
        <div className="grid gap-8 rounded-[2rem] border border-[#0b1736]/10 bg-white/70 p-7 dark:border-white/10 dark:bg-white/[0.04] lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[#008080]">
              <ShoppingBag size={16} weight="bold" /> FeniX Commerce
            </div>
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 opacity-60">
              ফেনীর approved seller ও business থেকে products discover করুন। Guest checkout-ও
              supported.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href="/commerce/sell"
              className="inline-flex items-center gap-2 rounded-xl bg-[#008080] px-5 py-3 text-sm font-bold text-white"
            >
              <Storefront size={18} weight="bold" /> Sell on FeniX <ArrowRight size={16} />
            </Link>
            <Link
              href="/commerce/cart"
              className="inline-flex items-center gap-2 rounded-xl border border-[#0b1736]/10 px-5 py-3 text-sm font-bold dark:border-white/10"
            >
        ...
        >
          ...
          <button
            type="submit"
            className="rounded-xl bg-[#0b1736] px-5 text-sm font-bold text-white dark:bg-white/[0.12]"
          >
            Search
          </button>
        </form>
        ...
        <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="inline-flex items-center gap-2 text-xs opacity-50">
            <ShieldCheck size={16} weight="bold" /> Approved sellers are verified separately
          </div>
        </div>
        ...
        {error ? (
          ...
          <div className="mt-6 rounded-[2rem] border border-[#0b1736]/10 bg-white/70 p-12 text-center dark:border-white/10 dark:bg-white/[0.04]">
            <ShoppingBag size={40} weight="duotone" className="mx-auto opacity-30" />
            <h3 className="mt-4 text-xl font-bold">No published products yet</h3>
            <p className="mt-2 text-sm opacity-55">
              Approved sellers can create product drafts from the seller area.
            </p>
          </div>
        )}
      </section>
